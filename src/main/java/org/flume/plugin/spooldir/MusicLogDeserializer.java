...
public class MusicLogDeserializer implements EventDeserializer {

	  private static final Logger logger = LoggerFactory.getLogger
	      (MusicLogDeserializer.class);
	  ...
	  public static final int MAXLINE_DFLT = 2048000;

	  MusicLogDeserializer(Context context, ResettableInputStream in) {
	    this.in = in;
	  ...
	  public Event readEvent() throws IOException {
	    ...
	    if(message!=null){
	    	//logger.info(message);
		    message = message.replace("u'", "'").replace("None", "\"\""); 
		    //[INFO 2015-07-21 14:32:28,119 /opt/music/service/xmlrpc/online/ms_proxy.py:37 @ 30472] - visitlog|
		    if(!message.startsWith("[INFO")){
		    	logger.info("Msg not start with [INFO ,pass");
		    	message = null;
		    }else{ 
			    ...
			    //logger.debug("get message: " + message);
			  	try{
			      	...
			      	boolean fromCache = response.getString("fromCache").equals("1");
			      	if(fromCache){
			  		...
			  		ex.printStackTrace();
			  		logger.warn("log json parse error,the msg is {}!",
			  				message);
			  	}
		    }
	    }
	    if (message == null) {
	      ...
	      return EventBuilder.withBody(message, outputCharset);
	    }
	  }
	  ...
	  public static class Builder implements EventDeserializer.Builder {
	    ...
	    public EventDeserializer build(Context context, ResettableInputStream in) {
	      return new MusicLogDeserializer(context, in);
	    }
