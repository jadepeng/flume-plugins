...
import static org.flume.plugin.spooldir.SpoolDirectorySourceConfigurationConstants.*;
...
//  private List<Optional<FileInfo>> currentFiles = Lists.newArrayList();
  ...
  private ReliableSpoolingFileEventReader(File spoolDirectory,
      ...
      ConsumeOrder consumeOrder,int readerIndex) throws IOException {
    ...
    try {
      File canary = File.createTempFile("flume-spooldir-perm-check-", ".canary"+readerIndex,
          spoolDirectory);
    ...
    this.metaFile = new File(trackerDirectory, metaFileName+readerIndex);
...
//    }
  ...
  private void deleteCurrentFile(File fileToDelete) throws IOException {
    ...
    // now we no longer need the meta file
    deleteMetaFile();
    logger.info("delete file {} done!", fileToDelete);
  }
  ...
  private Optional<FileInfo> getNextFile() {
      ...
      FileFilter filter = new FileFilter() {
        public boolean accept(File candidate) {
          ...
          if ((candidate.isDirectory()) ||
            ...
            CandidateFilesUtil.exist(fileName)) {
            return false;
          }
          return true;
        }
    ...
    // 加锁，防止多线程选到同一文件
    Lock lock = CandidateFilesUtil.lock();
    ...
    if (consumeOrder == ConsumeOrder.RANDOM) { // Selected file is random.
      ...
      for (File candidateFile: candidateFiles) {
		 if(CandidateFilesUtil.exist(candidateFile.getName())){
			 continue;
		 }
	    ...
	    if(selectedFile.exists() && !CandidateFilesUtil.exist(selectedFile.getName())){
	    	return openFile(selectedFile);
	    }
    }catch(Exception ex){
		return getNextFile();
  ...
  private Optional<FileInfo> openFile(File file) {    
    try {
      ...
      CandidateFilesUtil.add(file.getName());
  ...
  public static class Builder {
    ...
    public Builder readerIndex(int readerIndex) {
